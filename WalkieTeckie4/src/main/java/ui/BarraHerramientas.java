 ...
 * @author roder
 */
import static main.EstadoJuego.MENU;
import static main.EstadoJuego.SetGameState;
...
public class BarraHerramientas extends Barra {
	...
	public BarraHerramientas(int x, int y, int width, int height, Edicion editing) {
	...
	private void initPathImgs() {
		pathStart = CargarGuardar.getSpriteAtlas().getSubimage(7 * 32, 2 * 32, 32, 32);
		pathEnd = CargarGuardar.getSpriteAtlas().getSubimage(8 * 32, 2 * 32, 32, 32);
	}
	...
	private void initButtons() {
		...
		bGrass = new MyButton("Pasto", xStart, yStart, w, h, i++);
		bWater = new MyButton("Agua", xStart + xOffset, yStart, w, h, i++);
		...
		bPathStart = new MyButton("Inicio Camino", xStart, yStart + xOffset, w, h, i++);
		bPathEnd = new MyButton("Final Camino", xStart + xOffset, yStart + xOffset, w, h, i++);

...
public class CargarGuardar {
	...
	public static String saveFolder = "PartidasGuardadas";
	public static String levelFile = "nivel.txt";
	...
	public static BufferedImage getSpriteAtlas() {
		...
		InputStream is = CargarGuardar.class.getClassLoader().getResourceAsStream("spriteatlas.png");
	...
	public static void CreateLevel(int[] idArr) {
		if (lvlFile.exists()) {
			System.out.println("El archivo: " + lvlFile + " ya existe");
			return;
		} else {
			try {
	...
	public static void SaveLevel(int[][] idArr, PathPoint start, PathPoint end) {
		if (lvlFile.exists()) {
			WriteToFile(Utilidades.TwoDto1DintArr(idArr), start, end);
		} else {
			System.out.println("El archivo: " + lvlFile + " no existe ");
			return;
		}
	}
	...
	public static int[][] GetLevelData() {
		if (lvlFile.exists()) {
			ArrayList<Integer> list = ReadFromFile();
			return Utilidades.ArrayListTo2Dint(list, 20, 20);
